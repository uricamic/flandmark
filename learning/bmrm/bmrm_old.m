...
%   .BufSize [1skx1] Size of cutting plane buffer in MB (default 100).
% 
% Outputs:
...
if options.verb
    fprintf('%4d: Fp=%f, Fd=%f, R=%f\n', nIter, R+0.5*lambda*norm(W)^2, -inf, R);
end
...
while exitflag == -1
    ...
    if options.verb
        fprintf('%4d: Fp=%f, Fd=%f, (Fp-Fd)=%f, (Fp-Fd)/Fp=%f, R=%f, nCP=%d, nzA=%d\n', ...
                nIter, Fp, Fd, Fp-Fd,(Fp-Fd)/Fp, R, nCP, nzA);
    end
