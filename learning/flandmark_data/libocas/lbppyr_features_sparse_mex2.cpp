...
/*  Features = (int64_t*)mxGetPr(plhs[0]);*/
  ...
  TmpFeatPtr = TmpFeatures;
  double percent_sparse = 0.01;
  mwIndex *irs, *jcs;
  mxLogical *sr;
  mwSize nzmax = (mwSize)ceil(nData*nDim*percent_sparse);
    ...
    liblbp_pyr_features(&TmpFeatures[nDim*i], nDim, win, win_H, win_W);
      ...
      for (j = 0; j < nDim; ++j)
      {
          ...
          if (TmpFeatPtr[j])
          {
              ...
              if (k > nzmax)
              {
                  mexPrintf("k > nzmax, reallocating...\n");
                  mwSize oldnzmax = nzmax;
                  percent_sparse += 0.01; // plus 1 percent
                  nzmax = (mwSize)ceil(nDim*nData*percent_sparse);
                  if (nzmax == oldnzmax) ++nzmax;
                  mxSetNzmax(plhs[0], nzmax);
                  mxSetPr(plhs[0], (double*)mxRealloc(sr, nzmax*sizeof(*sr)));
                  //mxSetData(plhs[0], (mxLogical*)mxRealloc(sr, nzmax*sizeof(*sr)));
                  mxSetIr(plhs[0], (mwIndex*)mxRealloc(irs, nzmax*sizeof(irs)));
                  sr = mxGetLogicals(plhs[0]);
                  irs = mxGetIr(plhs[0]);
              }
              sr[k] = 1;
