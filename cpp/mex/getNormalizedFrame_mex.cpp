...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	model->p_height = dims[0];  model->p_width = dims[1];

	//convert matlab image matrix into ipl_image
	//mexPrintf("convert matlab image matrix into IplImage.\n");
	...
	//mexPrintf("create model.\n");
	model->sf = (float*)malloc(2*sizeof(float));
	model->croppedImage = cvCreateImage(cvSize(model->p_width, model->p_height), IPL_DEPTH_8U, 1);
	model->resizedImage = cvCreateImage(cvSize(model->p_width, model->p_height), IPL_DEPTH_8U, 1);

	//mexPrintf("call getNormalizedImageFrame...");

	//if (getNormalizedImageFrame(ipl_image, bbox, model->bb, model->normalizedImageFrame, model))
	if (getNormalizedImageFrame(ipl_image, bbox, p_bb, p_face_image, model))
	{
		//    mexErrMsgTxt("Error ?\n");
	}
	//mexPrintf(" done.\n");
