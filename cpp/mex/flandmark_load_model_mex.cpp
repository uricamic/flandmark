...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    if (nrhs < 2 || nrhs > 3)
    {
        mexErrMsgTxt("Improper number of input arguments.\n\n"
                "FLANDMARK_LOAD_MODEL loads model for flandmark detector and writes it to file.\n\n"
                "Synopsis: \n"
                "libfld_load_model(model, fname)\n"
                "\n"
                "Input: \n"
                "  model [structure]\n"
                "  fname [string]\n"
    ...
    model->bb = 0;
    model->sf = 0;
    model->croppedImage = 0;
    model->normalizedImageFrame = 0;
    model->resizedImage = 0;
    ...
    //const char *fname = "model_gdisp.dat";
    flandmark_writeModel(fname, model);
