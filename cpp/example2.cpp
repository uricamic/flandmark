...
IplImage* getCameraFrame(CvCapture* &camera, const char *filename = 0)
{
    IplImage *frame = 0;
    int w, h;
...
void detectFaceInImage(IplImage *orig, IplImage* input, CvHaarClassifierCascade* cascade, FLANDMARK_Model *model, int *bbox, float *landmarks)
{
    // Smallest face size.
...
int main( int argc, char** argv )
{
    ...
    if (argc > 1)
    {
        infname = argv[1];
        printf("infname = %s\n", infname);
        video = (isdigit(infname[0])) ? false : true;
        if (video)
        {
